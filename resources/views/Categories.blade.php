            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="flex flex-col md:flex-row gap-4">
                    <div class="relative flex-grow">
                        <input type="text" id="searchInput" placeholder="Search by subcategory name..."
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-[15px] bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-[700px] scale-95 opacity-0 translate-y-4 sm:translate-y-0 duration-300"
                ...
                <!-- Header -->
                <div class="flex justify-between items-center p-6 pb-0">
                    <h2 class="text-xl sm:text-2xl font-semibold text-[#213268]">IMPORT SUB CATEGORIES</h2>
                    <button class="close-modal p-2 hover:bg-gray-100 rounded-full transition-colors duration-200">
                        <svg class="w-6 h-6 text-[#757575]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="text-gray-600 text-sm bg-blue-50 p-4 rounded-lg">
                                ...
                                <ul class="list-disc pl-5 space-y-1">
                                    ...
                                    <li>Required columns: Sub Category Name, Asset Type</li>
                    ...
                    <div class="p-6">
                        <div class="space-y-6">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-sm text-gray-500" id="category-preview-count">0 items found</span>
                            </div>
                                ...
                                <table class="w-full">
                                    <thead class="sticky top-0 bg-[#213268] text-white">
                                        <tr>
                                            ...
                                            <th class="p-3 text-left text-xs font-semibold">Sub Category Name</th>
                                            <th class="p-3 text-left text-xs font-semibold">Description</th>
                                        </tr>
                                    </thead>
                    ...
                    <div class="p-6">
                        <div class="space-y-6">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="mt-2 text-sm text-gray-600">Your sub categories have been imported successfully.</p>
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
                ...
                if (this.files && this.files[0]) {
                    ...
                    if (!['xlsx', 'xls', 'csv'].includes(fileExt)) {
                        if (categoryExcelError) {
                            categoryExcelError.textContent = 'Invalid file type. Please upload an Excel file (.xlsx, .xls) or CSV file.';
        ...
        if (categoryPreviewBtn) {
            categoryPreviewBtn.addEventListener('click', function() {
                if (!categoryExcelFile || !categoryExcelFile.files || !categoryExcelFile.files[0]) {
                    if (categoryExcelError) {
                        categoryExcelError.textContent = 'Please select a file first.';
                        categoryExcelError.classList.remove('hidden');
                    }
                    return;
                }
                ...
                reader.onload = function(e) {
                    try {
                        ...
                        if (categoryExcelLoading) categoryExcelLoading.classList.add('hidden');
                        if (categoryExcelError) {
                            categoryExcelError.textContent = 'Error processing file: ' + error.message;
                            categoryExcelError.classList.remove('hidden');
                        }
                    }
                };
                ...
                reader.onerror = function() {
                    ...
                    if (categoryExcelLoading) categoryExcelLoading.classList.add('hidden');
                    if (categoryExcelError) {
                        categoryExcelError.textContent = 'Error reading the file. Please try another file.';
                        categoryExcelError.classList.remove('hidden');
                    }
                };
        ...
        function processCategoryExcelData(data) {
            ...
            rows.forEach((row, rowIndex) => {
                ...
                if (!item.asset_type) {
                    warnings.push(`Row ${rowIndex + 2}: Missing Asset Type`);
                } else if (item.asset_type !== 'medical' && item.asset_type !== 'non_medical') {
                    warnings.push(`Row ${rowIndex + 2}: Invalid Asset Type "${item.asset_type}". Must be "medical" or "non_medical"`);
                }

                if (!item.subcategory_name) {
                    warnings.push(`Row ${rowIndex + 2}: Missing Sub Category Name`);
                }
            ...
            Object.entries(subcategoryNameMap).forEach(([key, rows]) => {
                if (rows.length > 1) {
                    ...
                    warnings.push(`Duplicate Sub Category "${subcategoryName}" for Asset Type "${assetType}" found in rows: ${rows.join(', ')}`);
                }
            });
        ...
        categoryImportForm?.addEventListener('submit', function(e) {
            ...
            .then(data => {
                ...
                if (data.success === true || (data.status >= 200 && data.status < 300)) {
                    ...
                    showToast('Sub categories imported successfully!', 'success');
