                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex flex-col">
                            ...
                            <span
                                class="font-medium">{{ isset($maintenance['start_date']) ? \Carbon\Carbon::parse($maintenance['start_date'])->locale('id')->isoFormat('D MMMM Y') : 'N/A' }}</span>
                        </div>
                        <div class="flex flex-col">
                            <span class="text-sm text-gray-500">Tanggal Selesai</span>
                            <span
                                class="font-medium">{{ isset($maintenance['end_date']) ? \Carbon\Carbon::parse($maintenance['end_date'])->locale('id')->isoFormat('D MMMM Y') : 'N/A' }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            <div class="flex flex-col">
                                ...
                                <span
                                    class="font-medium">{{ isset($maintenance['maintenance_report']['maintenance_date']) ? \Carbon\Carbon::parse($maintenance['maintenance_report']['maintenance_date'])->locale('id')->isoFormat('D MMMM Y') : 'N/A' }}</span>
                            </div>

                            <div class="flex flex-col">
                                <span class="text-sm text-gray-500">Dilaporkan Oleh</span>
                                <span
                                    class="font-medium">{{ $maintenance['maintenance_report']['reported_by_employee_number'] ?? 'N/A' }}</span>
                            </div>

                            <div class="flex flex-col">
                                <span class="text-sm text-gray-500">Laporan Dibuat Pada</span>
                                <span
                                    class="font-medium">{{ isset($maintenance['maintenance_report']['created_at']) ? \Carbon\Carbon::parse($maintenance['maintenance_report']['created_at'])->locale('id')->isoFormat('D MMMM Y') : 'N/A' }}</span>
                            </div>
                        </div>
                            ...
                            <tbody>
                                @foreach(is_array($maintenance['history']) ? $maintenance['history'] : [] as $historyItem)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="p-3 text-xs border-t border-[#EEF1F4]">
                                            {{ $historyItem['user_name'] ?? 'N/A' }}
                                        </td>
                                        <td class="p-3 text-xs border-t border-[#EEF1F4]">
                                            {{ ucfirst(str_replace('_', ' ', $historyItem['field_name'] ?? 'N/A')) }}
                                        </td>
                                        <td class="p-3 text-xs border-t border-[#EEF1F4]">
                                            {{ $historyItem['old_value'] ?? 'N/A' }}
                                        </td>
                                        <td class="p-3 text-xs border-t border-[#EEF1F4]">
                                            {{ $historyItem['new_value'] ?? 'N/A' }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
