                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="date" name="as_of_date"
                                        ...
                                        value="{{ $as_of_date ?? now()->format('Y-m-d') }}">
                                </div>
                            </div>
                    ...
                    @if(isset($pagination) && $pagination && $pagination['total_pages'] > 1)
                            ...
                            'asset_type' => $asset_type ?? ''
                                        ...
                                        <div class="relative mt-4 md:mt-0">
                                            <select name="limit" form="depreciationFilterForm"
                                                onchange="document.getElementById('depreciationFilterForm').submit()"
                                                class="border border-[#D8DAE5] rounded-md py-1 px-3 pr-8 appearance-none text-[#213268] text-sm">
                                                <option value="10" {{ ($pagination['per_page'] ?? 10) == 10 ? 'selected' : '' }}>10 per
                                                    halaman</option>
                                                <option value="25" {{ ($pagination['per_page'] ?? 10) == 25 ? 'selected' : '' }}>25 per
                                                    halaman</option>
                                                <option value="50" {{ ($pagination['per_page'] ?? 10) == 50 ? 'selected' : '' }}>50 per
                                                    halaman</option>
                                                <option value="100" {{ ($pagination['per_page'] ?? 10) == 100 ? 'selected' : '' }}>100 per
                                                    halaman</option>
                                            </select>
                                            <div
                                                class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-[#213268]">
